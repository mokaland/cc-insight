...
| 問題 | 原因 | 修正 |
|------|------|------|
| 日報送信後にフォームがクリアされない | 送信成功後にuseEffectで既存レポートが再充填される | justSubmittedフラグで再充填を防止 |
...
| 日付 | 変更内容 | コミット |
|------|---------|---------|
| 2025-01-11 | 日報送信後にフォームがクリアされないバグ修正 | 811b8c2 |
| 2025-01-11 | マイページに進化可能通知＋直接エナジー投資機能 | 1e043fb |
| 2025-01-11 | 開発メモファイル追加 | 186839f |
...
```
docs/DEVELOPMENT_NOTES.md を読んでプロジェクトの状況を把握して
```
...
**重要**: このファイルは開発の「記録」として機能します。

### Claudeへの指示
- 機能追加・バグ修正など**重要な変更をコミットしたら、必ずこのファイルの変更履歴を更新すること**
- 新しい実装ルールや注意点が判明したら、該当セクションに追記すること
- これはデフォルトの動作として行うこと（ユーザーからの指示を待たない）
