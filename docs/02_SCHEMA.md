...
**定義ファイル**: `lib/firestore.ts` (488-517行目), `lib/auth-context.tsx` (20-34行目)
...
**定義ファイル**: `app/dm/page.tsx` (26-35行目)
...
## 3. ⚠️ 要修正（Warning）：技術的負債

### 3.1 型定義の不整合

#### 🔴 重大: DMMessage インターフェースにフィールド欠落

**場所**: `app/dm/page.tsx` (26-35行目)
...
interface DMMessage {
  ...
  createdAt: Timestamp;
  isAdmin: boolean;
  // ❌ read, readAt, participants が欠落
}
```

**実際に保存されるデータ**（194-204行目）:
```typescript
...
**問題**: 型定義と実際のデータ構造が不一致。TypeScriptの型安全性が損なわれている。

---

#### 🟠 中程度: User と UserProfile の重複定義

**場所**: 
- `lib/firestore.ts` (488-517行目): `User` インターフェース
- `lib/auth-context.tsx` (20-34行目): `UserProfile` インターフェース

両方が同じ `users` コレクションを参照しているが、フィールドが微妙に異なる。

| フィールド | User | UserProfile |
|-----------|------|-------------|
| `profileImage` | ❌ | ✅ |
| `gender` | ✅ | ❌ |
| `ageGroup` | ✅ | ❌ |
| `guardians` | ✅ | ❌ |
| `guardianProfile` | ❌ | ❌（別途定義） |
| `snsAccounts` | ✅ | ❌ |

**推奨**: 単一のソースオブトゥルースとして統合。
...
1. **DMMessage インターフェースを修正**: `read`, `readAt`, `participants` フィールドを追加
2. **User と UserProfile を統合**: 単一の型定義に統一
3. **any 型の排除**: 特に `daily-login-bonus.ts` の Timestamp 型

### 優先度：中

4. **Firestore セキュリティルールとの整合性確認**
5. **インデックス定義の文書化**（`firestore.indexes.json` との照合）

### 優先度：低

6. **guardianProfiles コレクションの存在確認**
7. **SNS承認状態の簡素化検討**

---

*このドキュメントはソースコードから自動生成されました。実際のFirestoreデータとの差異がある可能性があります。*
