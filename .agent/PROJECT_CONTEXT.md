...
| カテゴリ | 内容 |
|----------|------|
| **Cron認証バグ修正** | `x-vercel-cron`ヘッダーが現在のVercelでは送信されない問題を発見 |
| **User-Agent認証に変更** | `vercel-cron/1.0` User-Agentで認証する方式に修正 |
| **staging→main マージ** | 100ファイル以上の大規模アップデートを本番デプロイ |
...
### Vercel Cron認証 ⭐ 2026-01-15発見
- ❌ `x-vercel-cron`ヘッダー: **現在のVercelでは送信されない**（古い情報に注意）
- ✅ `User-Agent: vercel-cron/1.0`: Vercel Cronからのリクエストを判定する公式方法
- ✅ `CRON_SECRET`設定時: Vercelが自動で`Authorization: Bearer $CRON_SECRET`を送信
