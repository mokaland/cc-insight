...
function FullScreenOverlay({
  ...
  useEffect(() => {
    ...
    // touchmoveイベントをブロック（モーダル内のスクロールは許可）
    const preventTouchMove = (e: TouchEvent) => {
      const target = e.target as HTMLElement;
      // .allow-scrollクラスを持つ要素内のスクロールは許可
  ...
  const content = (
    <>
      {/* 背景レイヤー - セーフエリア外まで確実にカバー */}
      <div
        className={`fixed ${bgGradient || bgColor}`}
        style={{
          position: 'fixed',
          top: '-200px',
          left: '-200px',
...
export default function GuardianSummoning({ userId, onComplete }: GuardianSummoningProps) {
  ...
  if (step === 'prologue') {
    ...
    return (
      <FullScreenOverlay bgColor="bg-black">
  ...
  if (step === 'complete' && selectedGuardian) {
    ...
    return (
      <FullScreenOverlay bgGradient="bg-gradient-to-b from-slate-950 via-purple-950 to-slate-950">
        <div className="min-h-full flex items-center justify-center p-4 relative">
